...
author:
- MSc. Patrick Roelli, Computational Biologist 2 - Computational Biology^[10x Genomics,
...
ElbowPlot(breast_cancer)
```

#### Clustering

It's important to note that the number of clusters is not only defined by the biology, the technical variation as well as the hyperparamters chosen in the algorithms.
There is one particular value, resolution, that can be changed to zoom in and out of the clustering. Here we use the default value, 0.8
```{r}
breast_cancer <- FindNeighbors(breast_cancer, reduction = "pca", dims = 1:30)
breast_cancer <- FindClusters(breast_cancer, verbose = FALSE, resolution = 0.8)
...
DimPlot(breast_cancer, reduction = "umap", label = FALSE,) +
  labs(color = "Cluster")
FeaturePlot(breast_cancer, features = c('nFeature_Spatial','pct.mito'), dims = 1:2)
```
...
We can also look at these data interactively. This function can be a little slow but also very useful to visualize expression in different projection spaces.
